package org.coastline.amber.server.dao;
...
public interface INodeInfoDao {
    ...
    @Select("<script>" +
            "SELECT " +
            "node, role, last_time, update_time, `${nodeInfoParam.infoItem}` " +
    ...
    @Insert("<script>" +
            ...
            "sync_full, sync_partial_ok, sync_partial_err, sync_full_min, sync_partial_ok_min, sync_partial_err_min, " +
            "keyspace_hits, keyspace_misses, keyspace_hits_ratio, used_cpu_sys, cpu_sys, used_cpu_user, cpu_user, " +
            "`keys`, `expires`, total_slow_log, slow_log, update_time) " +
            "VALUES " +
            "<foreach item='nodeInfo' collection='nodeInfoList' index='index' separator=','>" +
            "(#{nodeInfo.node}, #{nodeInfo.role}, #{nodeInfo.lastTime}, " +
            ...
            "#{nodeInfo.syncFull}, #{nodeInfo.syncPartialOk}, #{nodeInfo.syncPartialErr}, #{nodeInfo.syncFullMin}, #{nodeInfo.syncPartialOkMin}, #{nodeInfo.syncPartialErrMin}, " +
            "#{nodeInfo.keyspaceHits}, #{nodeInfo.keyspaceMisses}, #{nodeInfo.keyspaceHitsRatio}, #{nodeInfo.usedCpuSys}, #{nodeInfo.cpuSys}, #{nodeInfo.usedCpuUser}, #{nodeInfo.cpuUser}, " +
    ...
    @Update("UPDATE node_info_${clusterId} SET last_time = 0 WHERE last_time = 1")
    int updateLastTimeStatus(@Param("clusterId") Integer clusterId);
    ...
    void dropTable(@Param("clusterId") Integer clusterId);

    /**
     * 删除索引，数据量较大时，先删除索引再删除表
     * @param clusterId
     */
    @Select("DROP INDEX multiple_query ON node_info_${clusterId}")
    void deleteNodeInfoIndex(@Param("clusterId") Integer clusterId);
